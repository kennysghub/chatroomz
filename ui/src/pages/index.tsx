...
export default function Home() {
  ...
  return (
    <main className="flex flex-col p-4 w-full max-w-3xl m-auto bg-slate-100">
      <h1 className="text-4xl font-bold text-center mb-4">
        Lounge ({connectionCount})
      </h1>
      <ol
      ...
      >
        {messages.map((m) => {
          return (
            <li
              className="bg-blue-100 rounded-2xl p-4 my-2 break-all shadow-lg  "
              key={m.id}
            >
              <p className="text-small text-gray-400">{m.createdAt}</p>
              <p className="text-small text-gray-400">{m.port}</p>
              <p>{m.message}</p>
            </li>
          );
        })}
